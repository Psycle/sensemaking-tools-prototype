...
export async function learnTopics(
  ...
  let instructions = "Identify a " + depth + "-tiered hierarchical topic modeling of the following comments, and return the results as ";
  if (depth == 1) {
    instructions += "an array of strings.";
  } else if (depth == 2) {
    instructions += "an array of objects with keys name and subtopics, where subtopics points to an array strings (no nested objects).";
  }
  if (parentTopics !== undefined) {
    instructions += " The top level topics should be: " + JSON.stringify(['Infrastructure & Development', 'Social & Community Issues', 'Economy & Employment', 'Governance & Policy']) + ".";
